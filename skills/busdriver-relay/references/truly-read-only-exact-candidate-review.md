...
- As defense in depth for every Git observation, clear ambient `GIT_*`, disable system/global config, set `GIT_OPTIONAL_LOCKS=0`, `GIT_NO_LAZY_FETCH=1`, an empty `GIT_ALLOW_PROTOCOL`, and `core.fsmonitor=false`; use `--ignore-submodules=none` for status and `--no-ext-diff --no-textconv` for diff. Reject non-empty stderr and partial stdout even when Git exits zero.
