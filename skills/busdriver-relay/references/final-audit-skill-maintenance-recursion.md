...
12. **Let the interrupt reply be only the skill-library outcome.** When the user explicitly says the turn can only use memory/skill-management tools, the final response should not include a Delivery Mode progress update, PR checklist, pending backstop interpretation, or "next I will run..." workflow promise. Say which class-level skill/reference was patched (or "Nothing to save"), note any resulting repo↔installed drift as pending for a later Delivery Mode turn, and stop.
13. **Skill-library review prompts supersede freshly-arrived Delivery Mode events for that turn.** If an async delegation/backstop completion is visible immediately before or alongside a user request to review/update the skill library with a memory/skill-only tool scope, treat the skill-library request as the active task. Do not summarize, validate, or consume the async event in the same reply; record only the durable skill outcome and leave Delivery Mode continuation for a later turn.
14. **A skill-library review turn is not a Delivery Mode handoff.** When the user asks to review the conversation and update skills while explicitly allowing only memory/skill-management tools, use `skill_manage`/`memory` only, then stop with the skill-library outcome. Do not add a Delivery Mode status table, do not restate pending PR/backstop hashes, and do not promise the next repository action in that same reply; those belong to the later continuation turn.
15. **Treat “be active” skill-library prompts as permission to improve skills, not to resume delivery.** When the prompt says most sessions should produce at least one skill update and gives class-level skill-library shaping guidance, actively look for a loaded umbrella/reference to patch, but still obey the tool-scope boundary. Prefer one concise class-level patch or support-file pointer; do not create narrow one-session skills, and do not use the “be active” wording as permission to inspect repos, consume async events, or advance PR state in the same turn.
