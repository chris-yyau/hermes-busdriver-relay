...
8. **During an explicit skill-library review turn, only mutate the skill library.** If the user asks to review the conversation and update skills mid-delivery, do not continue GitHub/repo/PR actions in that same turn when they scope tools to memory/skill management. Make the consolidated installed-skill update, then report the resulting repo↔installed drift as pending Delivery Mode work for the next turn.
