...
`hermes-busdriver-agent-smoke` is the opt-in real-adapter smoke. It creates a throwaway repo and may consume provider quota/tokens. Codex was verified this way: it added `src/codex_smoke.txt`, postflight saw only that scoped file, verifier passed, HEAD was unchanged, and status remained `needs_busdriver_review`. OpenCode was also verified after fixing `agent-draft` to export `BUSDRIVER_PLUGIN_ROOT` and `BUSDRIVER_STATE_DIR=.opencode`; before that, the user's OpenCode Busdriver plugin fell back to `~/.config/opencode` and failed closed on missing gate scripts.
