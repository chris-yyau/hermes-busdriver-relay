...
8. **Freeze the exact candidate.**
   - Refresh the base with `git fetch --no-tags origin main`. Audit attributes and clean/process filters before staging or status: reject every `.gitattributes` or `info/attributes` file before any worktree-facing Git operation, and reject any configured `filter.<name>.clean` or `filter.<name>.process` driver.
