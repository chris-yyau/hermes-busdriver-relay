...
4. **Treat `hermes-busdriver-litmus-status` as status evidence, not a shell truthiness shortcut.** It may report useful `ok=true` / `status=stale_or_missing` JSON while the wrapper command path returns nonzero in some contexts. Parse the JSON status for evidence, and only let nonzero status block when the status helper itself is unavailable or malformed.
5. **Finalization locks are branch-keyed.** If a squash merge deletes the topic branch before lock release, recreate the topic branch at the saved PR head SHA, release with the original token, switch back to `main`, then delete the recreated branch. Verify `hermes-busdriver-lock status --pretty` returns `count: 0`.
6. **End with a final audit after docs/status refresh merges.** Verify clean synced `main...origin/main`, no open PRs, no topic refs, no relay locks, no fresh litmus/PR markers, installed skill diff clean, CURRENT_STATUS required evidence present and stale evidence absent, focused/full contract tests pass, py_compile passes, and smoke is `ok: true`.
...
→ smoke with resolved absolute plugin root
→ deliver verify
→ PR-mode Codex lead
...
→ merge
→ release branch-keyed lock, recreating topic branch if needed
→ final audit
```
