...
OpenCode+Go = secondary draft-adapter/sidecar candidate and existing plugin/Chinese-model lane until a challenger proves better gate/tool parity
Pi = Busdriver-compatible tool-harness / adapter candidate; evaluate with no-builtin-tools/custom-extension read-only and draft-only smokes before promoting or replacing OpenCode
...
- `scripts/hermes-busdriver-relay-brief`: Read-only compact status/roadmap helper for Telegram-style summaries. It reports local repo dirty/sync state, installed-skill drift, finalization contract status, the five active roadmap tasks (ADR0005 unlock contract, mutating PR-grind fix-loop design, marker interop contract, OpenCode adapter proof, and Status/UX), and a fail-closed `next_safe_slice` while keeping all commit/push/PR/merge/finalization/marker-write/programmatic-execution/non-Codex-adapter authority flags false.
