...
Codex = primary fallback implementation draft worker through Busdriver-approved handoff paths, never raw for repo-changing work
Pi = confirmed primary Busdriver-compatible tool-harness / adapter candidate; target-state until schema/wrapper/smoke/contract tests pass
...
8. opt-in `hermes-busdriver-agent-smoke` for real Codex adapter smoke tests;
