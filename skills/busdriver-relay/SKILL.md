...
- `scripts/hermes-busdriver-finalization-readiness`: Read-only finalization readiness helper that combines delivery status with Phase-0 status and emits a `hermes-busdriver-handoff/v0` envelope for Busdriver/Claude or an explicit operator finalizer. It forwards the Busdriver state dir through delivery status for litmus evidence, can pass optional relay-role resolution and sanitized litmus/pre-PR freshness evidence into the handoff evidence, propagates delivery-status litmus blockers into readiness, has no execute mode, and keeps commit/push/PR/merge/deploy/release/publish/marker-write authority false.
