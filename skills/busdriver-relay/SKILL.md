...
Codex = implementation-primary metadata and PR lead by user policy; never raw or production-dispatched for repo-changing work
OpenCode+Go = secondary/fallback draft-only metadata; contract proven in non-installed harnesses, with production dispatch blocked by `agent_containment_and_credential_broker_unavailable`
Pi = deferred adapter history; its explicit harness compatibility remains, but it is not the default, current, or preferred route
...
**User expectation for repo-changing work:** all ordinary repo implementation should use this split. Hermes remains the relay/router/verifier and explicit Delivery Mode operator, while Busdriver/Claude remains the sole canonical finalization authority. Codex is implementation-primary metadata and PR lead, OpenCode + Go is secondary/fallback draft-only metadata, Pi is deferred adapter history, and Cursor is the manual IDE sidecar. Route selection does not dispatch work. Direct Hermes edits are a temporary, explicitly named bootstrap/emergency exception only when building or repairing the relay/pipeline itself and the approved draft-worker path is not yet available; do not let that exception become the default workflow.
...
- asks about Codex-primary/OpenCode-fallback routing metadata, deferred Pi history, the current production agent blocker, or the Claude/Busdriver backstop;
...
**Current User Scope Policy (embedded preference):** Busdriver/Claude is the sole canonical workflow and finalization authority; `hermes-busdriver-relay` is the relay/router/verifier and explicit Delivery Mode operator. Codex is implementation-primary metadata and PR lead, OpenCode + Go is secondary/fallback draft-only metadata, Pi is deferred adapter history, and Cursor is the manual IDE sidecar. All relay roles remain non-dispatchable and unverified because no executable relay-role dispatcher exists. Preserve `avoid_coding_agent_for_review=true`; Codex same-provider review is non-dispatchable without a fresh independent-session contract. Do not put relay role config in `~/.claude/busdriver.json`; use relay config (default `~/.hermes/busdriver-relay/config.json`). Do not make Busdriver itself Claude-free unless the user explicitly asks; label relay-side equivalents as metadata, not Busdriver-native Claude runtime. When the user proposes a relay role change or asks why to add/replace an agent, verify live/docs-backed capability first, then answer.
...
- enable additional mutating agent adapters (Pi/OpenCode etc.) without a proven relay gate/smoke path and fresh user intent.
...
| Draft agent implementation seam | Agent may modify working tree but cannot finalize | Adapter contracts are proven only in non-installed harnesses. Production Pi/OpenCode dispatch is blocked by `agent_containment_and_credential_broker_unavailable`; route implementation to Busdriver/Claude until containment and credential brokering exist. |
...
- `scripts/hermes-busdriver-status`: Reports relay-owned configurable routing metadata for the 19-role map. Current defaults are Codex primary/PR lead, OpenCode secondary/fallback, Pi deferred, and Cursor manual IDE. Every role explicitly reports `programmatic_dispatch_allowed=false` and `adapter_verified=false` with a precise blocker because no production relay-role dispatcher exists. `avoid_coding_agent_for_review=true` remains the default, so same-provider Codex review degrades under `independent_review_session_contract_unavailable`. Status reports `role_policy=codex_primary_opencode_fallback_pi_deferred`. Relay config shape is root-level `{ "coding_agent": "codex", "avoid_coding_agent_for_review": true, "routes": { "relay.impl.primary": ["codex"], "relay.impl.fallback": ["opencode"], "relay.pr.lead": ["codex"], "relay.ide.manual": ["cursor"] } }`; invalid/malformed config remains fail-closed.
...
- `scripts/hermes-busdriver-agent-smoke`: Parser/authority-negative smoke surface. Production Pi/OpenCode launch is policy-blocked; historical real-smoke evidence does not prove containment or credential brokering. Codex/custom mutation routes are absent.
...
- `scripts/hermes-busdriver-relay-brief`: Read-only compact status/roadmap helper for Telegram-style summaries. Its catalog contains four active tasks plus one retained historical Pi-evidence row, and a fail-closed `next_safe_slice`; all commit/push/PR/merge/finalization/marker-write/programmatic-execution/non-Codex-adapter authority flags remain false.
...
Codex-primary/OpenCode-fallback is routing metadata, not an enabled production launcher. Pi is deferred adapter history. The non-installed Pi/OpenCode harnesses prove adapter schemas, scope enforcement, reconciliation, and authority-negative results only. Production `hermes-busdriver-gate` and `hermes-busdriver-agent-draft` stop before worker launch or credential copying with `agent_containment_and_credential_broker_unavailable`.
...
5. `hermes-busdriver-agent-draft` parser/fail-closed production responses plus non-installed Pi/OpenCode adapter test harnesses; production dispatch itself is blocked;
...
- activating support for additional non-Codex/non-Pi mutating agents without a dedicated adapter proof and fresh user intent.
...
- [ ] Keep Codex primary and PR-lead routing as metadata only, OpenCode + Go secondary/fallback draft-only, Pi deferred, Cursor manual, every role non-dispatchable, and same-provider Codex review blocked without a fresh independent-session contract.
