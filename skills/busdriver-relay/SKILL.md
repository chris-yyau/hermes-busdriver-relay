...
- `scripts/hermes-busdriver-agent-balance-plan`: Read-only planning envelope for balanced agent work. It describes one gated mutating draft lane plus parallel read-only review/status lanes, keeps every dispatch/programmatic/finalization/commit/push/PR/merge/deploy/release/publish/marker-write authority flag false, and does not call Codex/GitHub, launch subagents, mutate repos, or write markers.
...
- `scripts/hermes-busdriver-delivery-status`: Read-only Delivery Mode status envelope that combines repo state, Busdriver PR-grind source availability, relay capabilities, lock/run summaries, optional relay-role resolution evidence via `--relay-role` / `--relay-config`, sanitized, normalized/redacted, state-dir-aware read-only litmus/pre-PR freshness evidence, and optional PR-grind readiness output; it never authorizes or performs finalization. Stale/missing litmus markers are advisory warnings only when helper evidence is available and schema-safe; unavailable, malformed/schema-invalid, repo-mismatched, read-only-unsafe, authority-positive, or subprocess-failed litmus evidence is a fail-closed blocker. A non-dispatchable optional relay role is reported as a warning, not as merge/commit authority.
- `scripts/hermes-busdriver-deliver`: Fail-closed Delivery Mode dispatcher. Plan mode is read-only. Verify-only execution may run local verifier argv commands and write Hermes-owned run artifacts, and `execute --operation pr-grind` may wrap the read-only bounded PR-grind loop and write a Hermes-owned run artifact, but both still keep commit/push/PR/merge/deploy/release/publish/marker-write disabled. It forwards nested delivery-status timeouts and `--busdriver-state-dir-name` to litmus evidence checks. Every result carries a durable `hermes-busdriver-delivery-run/v0` envelope (`run_id`, phase, status, reason, repo/PR identity, authority flags, and artifact references); `--run-id` provides a stable run identity for operator/subagent/cron handoff while artifact filenames still add timestamp/PID uniqueness. `--mode status --run-id <id>` is a read-only lookup of the latest valid matching Hermes-owned run artifact; it validates versioned deliver/run envelopes plus fail-closed decision/authority metadata, preserves artifact repo/PR identity, returns artifact path plus sanitized metadata instead of verifier output tails, and does not run delivery-status, verifiers, PR-grind, or repo mutations. See `references/deliver-run-envelope-lessons.md`.
- `scripts/hermes-busdriver-litmus-status`: Read-only litmus/pre-PR marker freshness helper. It computes the current HEAD and Busdriver-style branch diff hash with the same plain `git diff` semantics as Busdriver's PR gate, fails closed instead of executing external diff/textconv/diff-driver configuration or hashing through `.gitattributes`, `$GIT_DIR/info/attributes`, or `core.attributesFile` diff selection, reports whether `.claude/litmus-passed.local`, `.claude/pr-codex-lead.local.json`, `.claude/pr-backstop-verdict.local.json`, and `.claude/pr-review-passed.local` are fresh by current Busdriver gate semantics, normalizes/fingerprints marker text and summarizes JSON fields instead of echoing raw contents, refuses state-dir symlink components or marker symlinks and refuses non-regular/oversized marker files, requires timestamp-fresh PR artifacts, treats empty PR diffs as unavailable, treats commit markers older than the current HEAD timestamp as stale, and never writes markers or grants finalization/commit/push/PR/merge/deploy/release/publish/marker-write authority.
- `scripts/hermes-busdriver-finalization-readiness`: Read-only finalization readiness helper that combines delivery status with Phase-0 status and emits a `hermes-busdriver-handoff/v0` envelope for Busdriver/Claude or an explicit operator finalizer. It forwards the Busdriver state dir through delivery status for litmus evidence, can pass optional relay-role resolution and sanitized litmus/pre-PR freshness evidence into the handoff evidence, embeds read-only `finalization_contract_status` output so downstream readiness consumers do not need a second helper call, embeds validated read-only `agent_balance_plan` lane-policy evidence while failing closed on malformed/non-read-only/authority-positive/timeout/subprocess-failed helper output, includes a `hermes-busdriver-dual-review-readiness/v0` envelope that reports programmatic litmus/pre-PR dual-review execution as unsupported while surfacing configured `relay.litmus.reviewer`, `relay.pr.lead`, and `relay.pr.backstop` routes as advisory evidence, propagates delivery-status litmus blockers into readiness, has no execute mode, and keeps dispatch plus commit/push/PR/merge/deploy/release/publish/marker-write authority false.
