...
- `scripts/hermes-busdriver-deliver`: Fail-closed Delivery Mode dispatcher. Plan mode is read-only. Verify-only execution may run local verifier argv commands and write Hermes-owned run artifacts, but still must keep commit/push/PR/merge/deploy/release/publish disabled. Every result carries a durable `hermes-busdriver-delivery-run/v0` envelope (`run_id`, phase, status, reason, repo/PR identity, authority flags, and artifact references); `--run-id` provides a stable run identity for operator/subagent/cron handoff while artifact filenames still add timestamp/PID uniqueness. See `references/deliver-run-envelope-lessons.md`.
