...
- `scripts/hermes-busdriver-delivery-status`: Read-only Delivery Mode status envelope that combines repo state, Busdriver PR-grind source availability, relay capabilities, lock/run summaries, optional relay-role resolution evidence via `--relay-role` / `--relay-config`, and optional PR-grind readiness output; it never authorizes or performs finalization. A non-dispatchable optional relay role is reported as a warning, not as merge/commit authority.
