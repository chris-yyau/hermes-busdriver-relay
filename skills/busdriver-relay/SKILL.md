...
Codex = current normal implementation draft worker through Busdriver-approved handoff paths, never raw for repo-changing work
Pi = constrained Busdriver-compatible tool-harness adapter lane after schema/wrapper/smoke/contract proof; no built-in mutating tools, `bd_*` only, ends at `needs_busdriver_review` or `blocked`
...
- asks Hermes to launch/coordinate **Codex** (current normal draft lane) or Pi adapter proof work through Busdriver;
...
| Draft agent implementation seam | Agent may modify working tree but cannot finalize | Allowed only inside `hermes-busdriver-gate preflight → agent → postflight`; result remains draft. Current normal lane is Codex; constrained Pi adapter lane is allowed only through the proven `bd_*` tool harness. |
...
- `scripts/hermes-busdriver-agent-draft`: Launcher that acquires the Hermes lock, runs gate preflight, executes **Codex** (current normal mutating draft surface), the constrained **Pi** `bd_*` adapter, or custom/noop test commands in draft mode under a best-effort PATH guard, runs postflight, and returns `needs_busdriver_review` or `blocked`.
...
- `scripts/hermes-busdriver-agent-smoke`: Optional real-agent adapter smoke; creates a throwaway repo and may consume provider quota/tokens. Codex and the constrained Pi adapter have been verified with it; other mutating agents require explicit adapter validation.
...
When Claude Code quota is unavailable, Hermes may independently call coding agents only through proven draft-gate adapter paths: Codex as the current normal lane, and Pi only through the constrained `bd_*` tool-harness adapter.
...
Use `hermes-busdriver-agent-draft` for the actual executable wrapper. It performs lock acquire/release, runs the gate pattern, launches Codex, constrained Pi, or noop/custom test commands, and saves run artifacts under Hermes-owned state. Use `hermes-busdriver-agent-smoke --agent codex` or `--agent pi` only as opt-in real adapter smoke because it consumes provider quota/tokens.

**Scope rule:** Although the underlying launcher code is written generically, respect the active adapter policy. Codex is the current normal draft lane; Pi is allowed only through the constrained `bd_*` adapter proof; do not enable, default to, or document additional mutating agents (OpenCode, droid, agy, grok, etc.) until the user requests a fresh adapter proof. This keeps the Hermes relay focused and avoids pretending generic tools are Busdriver-compatible.
...
4. `hermes-busdriver-gate` preflight/postflight around scoped draft-mode agents (Codex normal lane plus constrained Pi adapter);
5. `hermes-busdriver-agent-draft` for lock+gate+Codex or constrained Pi draft runs that return `needs_busdriver_review` or `blocked`;
...
- [ ] Respect current Codex normal lane plus constrained Pi adapter scope policy; additional mutating agents require dedicated adapter proof and fresh user intent.
