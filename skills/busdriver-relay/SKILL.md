...
```text
Hermes = recognition, Phase 0 discovery, JIT source reads, read-only status, router, user interaction, notification, verification/finalization under gates
Busdriver/Claude Code = workflow authority, gates, reviews, MCP/plugin routing, canonical pipeline semantics
Pi = default target draft adapter; contract proven in non-installed harnesses, but production dispatch is blocked by `agent_containment_and_credential_broker_unavailable`
OpenCode+Go = fallback/comparison target adapter; contract proven in non-installed harnesses, with the same production blocker
Codex = PR lead / review analysis by default; implementation only by explicit exception, never raw for repo-changing work
...
Zed = manual IDE sidecar; treat edits as human/manual dirty tree requiring Hermes/Busdriver verification
...
**User expectation for repo-changing work:** all ordinary repo implementation should use this split. Hermes should not become the primary coding agent that hand-edits repositories. It should coordinate the Busdriver-equivalent pipeline, preserve Pi-first/OpenCode-fallback as target routing policy, but route production implementation to Busdriver/Claude while the relay agent blocker is active, keep Codex primarily for PR lead/review and Claude Code as the backstop path, reconcile the resulting dirty tree, then verify/finalize. Direct Hermes edits are a temporary, explicitly named bootstrap/emergency exception only when building or repairing the relay/pipeline itself and the approved draft-worker path is not yet available; do not let that exception become the default workflow.
...
- asks about Pi-first/OpenCode-fallback target routing, the current production agent blocker, Codex review, or the Claude/Busdriver backstop;
...
**Current User Scope Policy (embedded preference):** Busdriver itself is the Claude-side workflow/control plane; `hermes-busdriver-relay` is the Hermes/Pi-side bridge/equivalent layer. Target draft routing is Pi-first with OpenCode as fallback/comparison, but both production launchers are currently non-dispatchable under `agent_containment_and_credential_broker_unavailable`; Codex should focus on PR lead / review by default, Claude Code owns the backstop path, and Codex should only implement by explicit exception. All relay-side decision/review anchors and voices (blueprint reviewers/arbiter, PR lead/backstop, council architect/pragmatist/critic/researcher/skeptic) should be configurable from a relay-owned JSON so Hermes can avoid using the same agent as both coder and reviewer. Do not put relay role config in `~/.claude/busdriver.json`; use relay config (default `~/.hermes/busdriver-relay/config.json`). Do not make Busdriver itself Claude-free unless the user explicitly asks; implement relay-side equivalents and label them as equivalents, not Busdriver-native Claude runtime. When the user proposes a relay role change or asks why to add/replace an agent, treat it as a question: verify live/docs-backed capability first, then answer. Do not write memory merely because the user floated a role idea; only persist after explicit confirmation or a clear durable preference.
...
- `scripts/hermes-busdriver-status`: Also reports relay-owned configurable equivalents for all relay reviewer/voice anchors, including `relay.blueprint.*`, `relay.pr.*`, `relay.council.*`, and `relay.litmus.reviewer`. These are logical Hermes agent/model routes from a separate relay config JSON (default `~/.hermes/busdriver-relay/config.json`), not shell CLI names, and never grant Busdriver-native Claude runtime authority. Current relay policy is Pi-primary implementation with OpenCode as fallback/comparison candidate, Codex as PR lead/review, Claude Code as backstop, Grok/Gemini read-only reviewer lanes, Zed as manual IDE, and UltraOracle as expert witness. Every default role explicitly declares boolean `programmatic_dispatch_allowed` and `adapter_verified`; omissions or wrong types degrade fail-closed rather than synthesizing permission. Defaults cover the 19-role map, `avoid_coding_agent_for_review=true`, and status reports `role_policy=pi_primary_opencode_fallback_codex_review` plus `review_independence_policy=avoid_coding_agent_for_review_roles`. Relay config shape is root-level `{ "coding_agent": "pi", "avoid_coding_agent_for_review": true, "routes": { "relay.impl.primary": ["pi"], "relay.impl.fallback": ["opencode"], "relay.pr.backstop": ["claude-code"], "relay.ide.manual": ["zed"] } }`; `coding_agent` must be a non-empty string, `avoid_coding_agent_for_review` must be boolean, `routes` must be an object, and each route value must be a non-empty string or an array of non-empty strings. Invalid/malformed config is reported as degraded/fail-closed (`selected_agent=null`) instead of silently falling back healthy; see `references/relay-configurable-roles-lessons.md`.
...
- `scripts/hermes-busdriver-relay-brief`: Read-only compact status/roadmap helper for Telegram-style summaries. It reports local repo dirty/sync state, installed-skill drift, finalization contract status, the five active roadmap tasks (ADR0005 unlock contract, mutating PR-grind fix-loop design, marker interop contract, Pi adapter proof, and Status/UX), and a fail-closed `next_safe_slice` while keeping all commit/push/PR/merge/finalization/marker-write/programmatic-execution/non-Codex-adapter authority flags false.
...
The Pi-first/OpenCode-fallback split is target routing policy, not an enabled production launcher. The non-installed harnesses prove adapter schemas, scope enforcement, reconciliation, and authority-negative results. Production `hermes-busdriver-gate` and `hermes-busdriver-agent-draft` instead stop before worker launch or credential copying with `agent_containment_and_credential_broker_unavailable`.
...
If a future slice implements reviewed containment and credential brokering, Pi remains the default target adapter and OpenCode the fallback/comparison target. Any successful draft must still end at `needs_busdriver_review` with commit/push/PR/merge/marker/deploy/release/publish/finalization authority false.
...
- [ ] Respect current Pi-default constrained adapter scope policy; Codex is fallback only when Pi is blocked or unsuited, and additional mutating agents require dedicated adapter proof and fresh user intent.
