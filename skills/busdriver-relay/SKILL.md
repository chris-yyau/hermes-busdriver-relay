...
Pi = default constrained implementation draft worker through the proven Busdriver-compatible `bd_*` adapter; no built-in mutating tools, ends at `needs_busdriver_review` or `blocked`
OpenCode+Go = candidate fallback implementation draft worker after Pi, plus China-model comparison lane; not Busdriver-compatible for repo-changing work until a Busdriver adapter/plugin is rebuilt and verified
...
- asks Hermes to launch/coordinate **Pi** as the default constrained draft lane, **OpenCode** as a proven fallback / China-model comparison lane, and **Codex** primarily for PR lead/review and **Claude Code** as backstop through Busdriver;
...
| Draft agent implementation seam | Agent may modify working tree but cannot finalize | Allowed only inside `hermes-busdriver-gate preflight → agent → postflight`; result remains draft. Current default lane is the constrained Pi `bd_*` adapter; Codex is allowed only as explicit fallback when Pi is blocked or unsuited. |
...
- `scripts/hermes-busdriver-agent-draft`: Launcher that acquires the Hermes lock, runs gate preflight, executes constrained **Pi** `bd_*` adapter drafts by default, **OpenCode** only after fallback adapter/plugin proof, **Codex** only for review/backstop or explicit implementation exceptions, or custom/noop test commands in draft mode under a best-effort PATH guard, runs postflight, and returns `needs_busdriver_review` or `blocked`.
...
- `scripts/hermes-busdriver-agent-smoke`: Optional real-agent adapter smoke; creates a throwaway repo and may consume provider quota/tokens. The constrained Pi adapter has been verified for the default lane; Codex smoke exists for legacy/review-exception paths; OpenCode and other mutating agents require explicit adapter validation before fallback use.
...
- `scripts/hermes-busdriver-finalization-contract-status`: Read-only ADR 0005 contract/capability matrix helper. It maps remaining finalization work to `policy_blocked` rows, keeps every authority/capability flag false, and surfaces ADR 0006 design evidence for programmatic dual-review and marker-interop without retiring either row. When adding top-level status row lists such as `authority_sources`, preserve compatibility fields like `required_authority_sources`, give each row the same fail-closed lifecycle fields consumers expect (`retired=false`, `implemented=false`, `capability_allowed=false`, `safe_to_execute_by_this_helper=false`), keep `current_evidence` empty unless a later approved helper actually verifies it, and extend recursive false-authority tests to cover the new rows; see `references/adr0005-authority-source-status-lessons.md`.
...
When Claude Code quota is unavailable, Hermes may independently call coding agents only through proven draft-gate adapter paths: Pi is the default constrained `bd_*` tool-harness adapter lane; OpenCode is the intended fallback / China-model comparison candidate once adapter proof exists; Codex is primarily for review/backstop and only implements by explicit exception.
...
Use `hermes-busdriver-agent-draft` for the actual executable wrapper. It performs lock acquire/release, runs the gate pattern, launches constrained Pi by default, OpenCode only after fallback proof, Codex only by review/backstop or explicit exception paths, or noop/custom test commands, and saves run artifacts under Hermes-owned state. Use `hermes-busdriver-agent-smoke --agent codex` or `--agent pi` only as opt-in real adapter smoke because it consumes provider quota/tokens.

**Scope rule:** Although the underlying launcher code is written generically, respect the active adapter policy. Pi is the default constrained `bd_*` adapter lane; OpenCode is the intended fallback / China-model comparison candidate only after adapter/plugin proof; Codex is review/backstop-focused by default. Do not enable, default to, or document other mutating agents (droid, agy, grok, etc.) until the user requests a fresh adapter proof. This keeps the Hermes relay focused and avoids pretending generic tools are Busdriver-compatible.
...
4. `hermes-busdriver-gate` preflight/postflight around scoped draft-mode agents (Pi-default constrained adapter plus Codex fallback);
5. `hermes-busdriver-agent-draft` for lock+gate+Pi-default constrained draft runs, or Codex fallback runs, that return `needs_busdriver_review` or `blocked`;
...
8. opt-in `hermes-busdriver-agent-smoke` for real Codex and constrained Pi adapter smoke tests;
9. advisory/user-facing routing that tells the user when finalization still needs Claude Code / Busdriver or stronger equivalent gates.
