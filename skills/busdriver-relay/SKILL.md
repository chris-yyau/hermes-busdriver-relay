...
For independent no-write audits of archived bundle + staged/unstaged/untracked state against exact current main, follow [references/read-only-forensic-archive-semantic-audit.md](references/read-only-forensic-archive-semantic-audit.md); classify semantic intent as already present, retired/dangerous, or genuinely rescuable, and treat legacy tests as policy evidence rather than automatic salvage candidates.

For interrupted Claude/Busdriver runs, max-turn continuation, and tmux-safe reproduction, follow [references/interrupted-claude-and-tmux-safety.md](references/interrupted-claude-and-tmux-safety.md).

For immutable frozen-diff reviews with a strict no-write boundary, follow [references/truly-read-only-exact-candidate-review.md](references/truly-read-only-exact-candidate-review.md).

For read-only audits where the hard problem is preserving a strict no-write observation boundary around a live dirty worktree—opening/closing seals, config and attributes preflight, sandboxed Git, and byte-manifest drift—follow [references/read-only-semantic-audit-legacy-dirty-worktrees.md](references/read-only-semantic-audit-legacy-dirty-worktrees.md).

For executor-retirement slices—removing a production route, relocating historical code to fixtures, updating trust/install/docs/test inventories, resealing digest closure without formatting churn, and completing latest-head PR grind—use `references/executor-retirement-and-policy-convergence.md` and `references/executor-retirement-pr-grind-lessons.md`.

For semantic classification of legacy Relay intent against current main—deduplicating patch stacks, mapping old tests to stronger or intentionally inverted policy, and deciding A/B/C salvage—use `references/legacy-dirty-worktree-semantic-audit.md`. Only C is implementation work; if C is empty, write no code.

Reference: `references/coding-workflow-authority-map-v0.1.md` is the canonical cross-agent authority map for ClaudeCode/Busdriver, Hermes, Codex, Pi, OpenCode, reviewer lanes, Delivery Mode, target-state wording, dirty-tree ownership, and reviewer data-egress gates. `references/full-role-map-dispatchability-lessons.md` captures the full-role-map lessons: resolver-ready roles are not automatically programmatically dispatchable, Pi is the sole executor route, OpenCode is non-executor history, stale inline reviewer threads may need explicit GraphQL resolution after fix-pushes, and merged repo skill-source edits must be compared back to the installed skill copy. `references/opencode-fallback-proof-audit-lessons.md` captures historical read-only OpenCode adapter-audit lessons without granting a current executor route or finalization authority. `references/gated-finalization-executor-opencode-lessons.md` captures the executor/OpenCode lesson: mutating Delivery Mode must be per-operation evidence-gated with default-deny reusable authority, marker production must use Busdriver trusted writer commands rather than raw `.claude/*` writes, and any unlock must update contract-status/readiness/docs/tests in the same slice. Historical/superseded role-policy evidence (not current routing; current authority: `references/coding-workflow-authority-map-v0.1.md`): `references/pi-adapter-implementation-lessons.md` captures Pi adapter implementation-specific workflow lessons: use a separate relay worktree when primary relay has WIP, implement adapter code in the relay repo rather than Pi itself, keep Pi target-state wording until proof passes, harden bd_* tools, and verify with fake-Pi contracts before optional real Pi smoke. `references/blueprint-arbiter-route-simplification.md` captures the user's preferred Busdriver blueprint-review arbiter route shape: ZenMux/gateway fable first, then fresh subscription opus, with no subscription-fable, inherited-model, or inline-arbiter fallback. `references/pr-grind-fail-closed-review.md` captures fail-closed PR-grind review lessons for relay status envelopes, litmus evidence validation, data-egress sanitization, and rerunning diff-hash-bound gates after amendments. For the end-to-end delivery-status/finalization litmus PR-mode slice, including recognized status validation, allowlisted-value redaction, nested wrapper timeout budgets, diff-hash-bound trusted writer sequencing, and PR-grind actionable-comment cleanup, see `references/delivery-litmus-pr-mode-lessons.md`. For smoke-summary finalization-contract evidence, delivery-status capability inventory, strict-false authority summaries, and avoiding stale PR-grind result files after fix pushes, see `references/pr41-smoke-contract-capability-lessons.md`. For strict helper-evidence validation regression coverage, full authority assertions, pytest param ids, latest-head PR-grind after reviewer fixes, and manual post-hook marker cleanup when Hermes finalizes outside Claude runtime, see `references/pr44-strict-helper-evidence-validation-lessons.md`. For the follow-up top-level delivery-status child envelope bug in finalization-readiness, including requiring child schema/read_only/boolean-ok validation, adding top-level delivery-status `read_only`, and documenting duplicated schema literals instead of importing executable helpers, see `references/pr45-delivery-envelope-validation-lessons.md`. For the safe completion/docs refresh after PR45, including replacing stale verification evidence across README/CURRENT_STATUS/settling-checks, full smoke for docs-only PRs, latest-head PR-grind on docs PRs, and claude-mem boundary logging, see `references/pr46-docs-status-refresh-lessons.md`. For PR47 docs/status refresh delivery lessons — scoped docs-only Codex drafts, recovering from malformed draft verifier commands by rerunning corrected postflight, using deliver verify while the repo is intentionally dirty, then latest-head PR-grind/merge/cleanup — see `references/pr47-docs-refresh-delivery-lessons.md`. For PR48 skill-source sync lessons — repo-vs-installed skill drift, sanitizing copied reference lessons for current policy, reviewer-feedback cleanup, and syncing the reviewed repo copy back into the installed skill — see `references/pr48-skill-source-sync-lessons.md`. For PR49 skill-sync delivery lessons — parameterized private path examples, deliver verifier `env` argv usage, branch-sensitive finalization lock release, and passing raw PR-grind loop payloads into finalization-readiness — see `references/pr49-skill-sync-delivery-lessons.md`. For PR50 docs/status refresh lessons — replacing stale verification evidence after merged skill/docs slices, dirty-tree smoke caveats, preserving finalization policy wording, and post-merge cleanup verification — see `references/pr50-docs-status-refresh-lessons.md`. For PR51 finalization unlock ADR lessons — ADR-only non-mutating dual-review/marker-interop contracts, per-file docs-link assertions, required dual-review schema fields, branch-sensitive lock release, and avoiding Claude plugin clone/cache edits during relay delivery — see `references/pr51-finalization-unlock-adr-lessons.md`. For PR52 ADR 0006 contract-status lessons — machine-readable ADR follow-up evidence while preserving `contract_adr` compatibility and false authority flags, deliver-verifier glob pitfalls, surgical `.codegraph` cache cleanup, and reviewer-bot quota comment interpretation — see `references/pr52-adr0006-contract-status-lessons.md`. For PR53–PR55 skill-sync continuation lessons — late async reviewer follow-up PRs, installed-skill-to-repo sync discipline, durable skill-reference tests, and final `diff -qr` verification — see `references/pr53-pr55-skill-sync-lessons.md`. For PR56 skill-sync delivery lessons — scoped verifier env hygiene for local git commit signing, independent backstop verdict `reviewed_diff_hash` augmentation, raw PR-grind loop evidence, and manual post-hook cleanup after Hermes finalizes outside Claude runtime — see `references/pr56-skill-sync-delivery-lessons.md`. For balanced agent work planning, single-mutating-worker/multi-read-only-lane policy, `*_allowed` naming pitfalls, metadata-only inventory, and Python bootstrap rationale for relay helpers, see `references/balanced-agent-work-planning-lessons.md`. Historical/superseded role-policy evidence (not current routing; current authority: `references/coding-workflow-authority-map-v0.1.md`): For the July 2026 relay-router role split — Busdriver/Claude authority, Hermes router/status, Codex primary fallback draft worker, OpenCode+Go secondary candidate, Grok/Gemini Hermes-side read-only reviewer lanes, Cursor manual sidecar, and Pi originally deferred — see `references/relay-router-agent-role-split.md`. Historical/superseded role-policy evidence (not current routing; current authority: `references/coding-workflow-authority-map-v0.1.md`): The later refined role map (blueprint reviewers `agy`/`claude-code`/`grok`, fresh Codex arbiter/critic/PR lead, Claude Code skeptic/PR backstop, Codex litmus, Codex→OpenCode implementation priority, and UltraOracle as optional expert witness) is retained at `references/relay-router-role-policy-2026-07.md`. For the historical Pi reassessment before the current Pi-only policy, use `references/pi-adapter-candidate-workflow.md` as superseded evidence only; it cannot override current Pi-only executor authority. For idle clean-main finalization-readiness slices, including reporting `no_finalization_candidate` instead of promoting stale litmus to a readiness blocker, preserving all authority flags false, and leaving the dirty tree for main Hermes/operator verification, see `references/idle-clean-finalization-readiness-lessons.md`. For read-only audits of idle finalization-readiness status slices, including acceptance criteria for no-candidate vs blocked-candidate semantics and fail-closed edge cases, see `references/idle-finalization-readiness-status-audit-lessons.md`. For read-only review of `docs/CURRENT_STATUS.md` refresh requirements, including which evidence to update, how to handle existing dirty WIP, and which finalization-policy wording must remain unchanged, see `references/current-status-readonly-review-lessons.md`. For finishing all remaining relay safe slices after a continuation request, including final Phase-0 sweeps, docs/status refreshes, installed-skill drift follow-up, corrected postflight after verifier quoting failures, PR-grind retry discipline, and final completion audit, see `references/relay-completion-sweep-lessons.md`. For PR60 skill-sync delivery lessons — exact installed-vs-repo `SKILL.md` byte alignment, explicit agent-draft `--repo`/`--plugin-root`, surgical verifier pycache cleanup and postflight rerun, evidence-bound verifier reruns, and post-PR marker cleanup before PR-grind — see `references/pr60-skill-sync-delivery-lessons.md`. For PR61–PR62 continuation lessons — post-merge skill-sync audit, exact CURRENT_STATUS evidence wording, live plugin-version correction, scoped docs postflight recovery, and resuming an interrupted docs refresh from dirty-branch state — see `references/pr61-pr62-continuation-lessons.md`. For PR63–PR64 skill-sync redaction lessons — sanitizing installed-skill references before repo sync, patching installed/repo copies together, adding private-path negative assertions, and following skill-sync merges with docs/status refreshes — see `references/pr63-pr64-skill-sync-redaction-lessons.md`. For post-delivery resource cleanup, distinguishing Hermes/Codex/Claude process families, safe temp/cache cleanup, and targeted relay-owned process-tree cleanup, see `references/relay-resource-cleanup-lessons.md`. For relay lock helper pitfalls — branch-keyed release after base checkout, `status` flag limitations, acquire JSON token shape, and requiring final `count=0` — see `references/lock-cli-usage-pitfalls.md`. For recurring Hermes-only disk cleanup, including silent script-only cron jobs, safe target paths, stdout/noise discipline, and avoiding other agents' durable state, see `references/periodic-disk-cleanup-cron-lessons.md`. For PR66 CURRENT_STATUS refresh lessons — evidence-only docs/status updates, verifier wording tolerance, avoiding literal `$BUSDRIVER_PLUGIN_ROOT` in `deliver --verifier`, branch-keyed lock release after squash merge, and final audit requirements — see `references/pr66-current-status-refresh-lessons.md`. For PR67 skill-sync reviewer-fix lessons — preserving fail-closed helper subprocess semantics in copied lessons, patching repo and installed skill copies together, recovering from accidental post-PR amend by turning it into a normal follow-up commit, and restarting latest-head PR evidence after a fix push — see `references/pr67-skill-sync-review-fix-lessons.md`. For PR68 late async follow-up lessons — classifying delayed subagent feedback against merged state, turning cheap still-applicable test hardening into a tiny follow-up PR, and treating already-deleted remote branches as cleanup-state checks — see `references/pr68-late-async-test-followup-lessons.md`. For read-only skill-sync audit/planning lanes after recent PR merges, including mid-audit WIP detection, installed-vs-repo drift classification, private-path redaction requirements, and CURRENT_STATUS follow-up scope, see `references/read-only-skill-sync-audit-lessons.md`. For convergence loops where skill-sync PRs make CURRENT_STATUS stale and docs refresh PRs reveal more installed-skill drift, use `references/skill-sync-current-status-convergence-lessons.md` before claiming final completion. For PR78-style skill-sync/pre-PR delivery continuation lessons — final whole-skill compare after subagents/main patches, including additional class-level installed drift in the same slice, using installed marketplace plugin version for smoke/status evidence, treating reviewer-bot actionable comments as PR-grind blockers even when labeled trivial, syncing skill-maintenance references before status refresh, and restarting latest-head evidence after reviewer-fix commits — see `references/pr78-skill-sync-pre-pr-lessons.md`. For post-merge convergence loops where a merged skill-sync PR is followed by new installed-only skill drift before the final `CURRENT_STATUS` refresh, including tiny follow-up skill-sync sequencing, clean-main litmus-status interpretation, finalization-contract-status invocation, and scoped private-path scans, see `references/post-merge-skill-drift-before-status-refresh.md`. For final-audit convergence loops where skill maintenance itself can create repeated installed↔repo drift, including batching lessons, avoiding recursion, requiring no newly-created skill drift before completion, and handling stale/false-positive PR-grind review-bot blockers after follow-up fixes, see `references/final-audit-skill-maintenance-recursion.md`. For skill-sync slices involving relay-router role policy references and copyable config snippets, including keeping repo/installed copies aligned, marking future-only roles as non-copyable until resolver support exists, validating sample configs against `hermes-busdriver-relay-role`, and restarting latest-head PR-grind after every reviewer-fix push, see `references/skill-sync-pr75-router-role-lessons.md`. For read-only roadmap audits and compact Status/UX brief slices, including classifying completed contract vs policy-blocked finalization tasks, auditing helper schema/count drift, dirty-tree precedence, and keeping OpenCode candidate metadata non-mutating, see `references/roadmap-readonly-audit-lessons.md`. For post-merge tiny follow-ups driven by late non-blocking reviewer feedback, including positive requested-repo helper tests, installed-skill drift created by skill-library updates, and avoiding ad hoc foreground sleep loops while waiting for reviewer bots, see `references/pr99-positive-helper-followup-lessons.md`. For one-line PR100-style skill-source sync slices, including whole-skill compare after copying installed drift, avoiding `&` in inline Python verifier snippets that trip Hermes shell-background guards, PR-mode `PASS-FAST` marker validation when current Busdriver supports it, and branch-keyed lock release after squash merge, see `references/pr100-skill-source-sync-lessons.md`. For post-PR100 task-list-only correction and PR101 convergence lessons — do not execute when the user only asked to write the next steps as a task list, sync any installed-only reference drift created by skill maintenance, use scoped git-signing overrides for verifier temp commits, and final-audit relay brief clean/idle — see `references/post-pr100-tasklist-and-pr101-convergence-lessons.md`. For continuing a relay slice through agent-draft timeouts, stale lock cleanup, ignored-cache postflight recovery, and readiness evidence embedding, see `references/agent-draft-readiness-evidence-lessons.md`. For ADR0005 authority-source status rows, including row lifecycle parity (`retired=false`/`implemented=false`), compatibility with `required_authority_sources`, recursive false-authority tests, Pi artifact-missing reconciliation, and PR-grind schema-consistency feedback, see `references/adr0005-authority-source-status-lessons.md`. For PR118-style skill-source sync slices after installed lessons drift ahead of repo source, including pinning durability tests to the intended SKILL entry wording rather than path presence alone, treating dirty-branch smoke as phase-inappropriate, using follow-up commits for reviewer-test hardening, and branch-keyed lock release after squash merges, see `references/pr118-skill-sync-delivery-lessons.md`. For PR106-style expanded skill-source sync slices, including adding durability tests for new reference files, fixing reviewer-bot documentation-surface comments, rerunning latest-head PR-mode review/PR-grind after fix pushes, and scoped git identity/signing env for final contract tests, see `references/pr106-expanded-skill-sync-pr-grind-lessons.md`. For late async-review blocker fixes during Pi adapter proof work, including re-reading runtime worktrees before patching, handling sibling amendments and ahead/behind PR branches, schema/blocked-artifact/stale-artifact/timeout hardening, Git-env and `bd_read` data-egress guards, and verification evidence discipline, see `references/pi-adapter-async-review-fix-lessons.md`. Historical/superseded role-policy evidence (not current routing; current authority: `references/coding-workflow-authority-map-v0.1.md`): For PR108-style Pi authority-map skill sync and delivery, including target-state Pi wording, hardened `bd_bash` Git status/diff allowlists, reviewer-fix PR-grind restarts, squash-merge cleanup pitfalls, branch-keyed lock cleanup, and final clean/idle audit expectations, see `references/pr108-pi-authority-sync-delivery-lessons.md`. Historical/superseded role-policy evidence (not current routing; current authority: `references/coding-workflow-authority-map-v0.1.md`): For PR109-style Pi adapter review/rebase delivery, including blocked artifacts, required authority flags, authority override order, read size guards, safe git status/diff argv hardening, rebase `ours`/`theirs` pitfalls, stale `REBASE_HEAD`, and adapter follow-up hardening, see `references/pr109-pi-adapter-review-rebase-lessons.md`. Historical/superseded role-policy evidence (not current routing; current authority: `references/coding-workflow-authority-map-v0.1.md`): For final PR109-style Pi adapter PR-grind loops, including forwarding `--scope-exclude`, matching gate glob semantics, newline-delimited scopes, structured missing-Pi blocked outputs, prompt-file launch to avoid argv/env limits, gitignore/fsmonitor/symlink/secret-path hardening, artifact scope cross-checks, GraphQL review-thread resolution, and merged-state handling, see `references/pr109-pi-adapter-final-pr-grind-lessons.md`. Historical/superseded role-policy evidence (not current routing; current authority: `references/coding-workflow-authority-map-v0.1.md`): For PR112-style Pi-default migration/dogfood, including auditing all Codex-default wording surfaces, preserving `--agent-cmd` implicit custom behavior, aligning balance-plan output, and proving stale reviewer comments are addressed before resolution, see `references/pr112-pi-default-dogfood-lessons.md`. Historical/superseded role-policy evidence (not current routing; current authority: `references/coding-workflow-authority-map-v0.1.md`): For live relay role-config restoration after target-policy vs resolver drift, including checking `~/.hermes/busdriver-relay/config.json`, restoring Pi-first/Codex-review routes, avoiding unsupported OpenCode fallback keys, and verifying `relay.pr.backstop` selected from `relay_config`, see `references/relay-live-config-restoration-lessons.md`. Historical/superseded role-policy evidence (not current routing; current authority: `references/coding-workflow-authority-map-v0.1.md`): For full role-map resolver/status implementation slices after live config gets ahead of repo support, including the 19-role map, Zed manual IDE correction, review-independence sensitivity, merged-PR worktree continuation, and dirty-tree smoke caveats, see `references/full-role-map-resolver-slice-lessons.md`.

Current role policy is defined by `references/coding-workflow-authority-map-v0.1.md` and the authority-negative `references/full-role-map-dispatchability-lessons.md`. Historical/superseded role-policy evidence (not current routing): `references/pi-adapter-implementation-lessons.md`, `references/relay-router-agent-role-split.md`, `references/relay-router-role-policy-2026-07.md`, `references/pr108-pi-authority-sync-delivery-lessons.md`, `references/pr109-pi-adapter-final-pr-grind-lessons.md`, `references/pr109-pi-adapter-review-rebase-lessons.md`, `references/pr112-pi-default-dogfood-lessons.md`, `references/relay-live-config-restoration-lessons.md`, and `references/full-role-map-resolver-slice-lessons.md`.

Previously unindexed historical lessons are retained at `references/deliver-verify-envelope-lessons.md`, `references/deliver-verify-pr-grind-followup-lessons.md`, and `references/pr30-finalization-guardrails-lessons.md`. Role resolver/status history is retained at `references/relay-configurable-roles-lessons.md` and the authority-negative `references/relay-role-status-integration-lessons.md`.

## Procedural/historical evidence — cannot override current authority in repository-root `docs/coding-workflow-authority-map.md`

These newly added references are retained as procedural/historical evidence only:

- `references/authority-chronology-only-exact-review.md`
- `references/authority-gated-finalization-review.md`
- `references/bounded-slice-review-method.md`
- `references/canonical-entry-boundary-reconstruction-and-executable-closure-review.md`
- `references/capability-sliced-doc-manifest-lessons.md`
- `references/defensive-trust-boundary-frozen-review.md`
- `references/delegated-runtime-placement-and-cleanup.md`
- `references/deliver-finalization-pr-grind-readonly-audit-lessons.md`
- `references/delivery-executor-finalization-review-lessons.md`
- `references/delivery-executor-slice-litmus-timeout-lessons.md`
- `references/delivery-executor-split-litmus-and-marker-lessons.md`
- `references/delivery-finalization-dogfood-edge-lessons.md`
- `references/delivery-finalization-group-splitting-litmus-lessons.md`
- `references/delivery-finalization-script-group-litmus-lessons.md`
- `references/delivery-mode-finalization-pitfalls.md`
- `references/delivery-mode-hook-preservation-and-backstop-lessons.md`
- `references/delivery-mode-hook-preserving-mutation-lessons.md`
- `references/delivery-mode-review-splitting-and-hardening-lessons.md`
- `references/deterministic-private-runtime-review-probes.md`
- `references/deterministic-restack-replay-review.md`
- `references/docs-skill-policy-consistency-review.md`
- `references/dual-review-marker-interop-readonly-audit-lessons.md`
- `references/durable-artifact-truth-and-review-closure.md`
- `references/ephemeral-self-hosted-required-checks.md`
- `references/exact-frozen-review-mutation-hermeticity-closure.md`
- `references/exact-github-action-annotation-pinact-review.md`
- `references/exact-prefreeze-review-repair-and-authenticated-runtime.md`
- `references/exact-prefreeze-tests-docs-hermeticity-review.md`
- `references/exact-private-runtime-packaging-closing-qa.md`
- `references/exact-stack-cas-fast-forward.md`
- `references/exact-tree-contract-docs-manifest-ownership-replay.md`
- `references/exact-tree-delivery-ci-truth-review.md`
- `references/exact-tree-hunk-rebalancing-and-readonly-replay.md`
- `references/exact-tree-review-concurrency-and-live-ci-reconciliation.md`
- `references/exact-tree-review-reseal-security-lessons.md`
- `references/exact-tree-roll-forward-and-restack-lessons.md`
- `references/exact-tree-security-runtime-closure.md`
- `references/final-review-remediation-patterns.md`
- `references/final-tree-contract-docs-completeness-review.md`
- `references/final-tree-review-triage-and-temporal-slicing.md`
- `references/final-tree-validity-and-tdd-remediation.md`
- `references/final-verdict-admissibility-and-ci-evidence-ledger.md`
- `references/finalization-authority-hardening.md`
- `references/finalization-contract-matrix-review-lessons.md`
- `references/finalization-delivery-loop-pitfalls.md`
- `references/finalization-github-reconciliation-and-env-hardening.md`
- `references/finalization-large-diff-split-and-review-retry-lessons.md`
- `references/finalization-review-loop-pitfalls.md`
- `references/finalization-review-pitfalls.md`
- `references/focused-stack-replay-ownership-closure.md`
- `references/frozen-contract-status-review-lessons.md`
- `references/frozen-correctness-negative-probe-patterns.md`
- `references/frozen-correctness-policy-ordering-and-step-truthfulness.md`
- `references/frozen-correctness-state-integrity-review.md`
- `references/frozen-dirty-snapshot-review.md`
- `references/frozen-production-transitive-ownership-lessons.md`
- `references/frozen-production-trust-boundary-ownership-review.md`
- `references/frozen-relay-consistency-negative-probes.md`
- `references/frozen-repair-worker-and-verification-isolation.md`
- `references/frozen-review-auth-docs-runtime-identity-hardening.md`
- `references/frozen-review-continuation-and-evidence-recovery.md`
- `references/frozen-security-review-blocker-tdd.md`
- `references/frozen-state-integrity-policy-ordering.md`
- `references/frozen-tests-docs-integrity-review.md`
- `references/frozen-tests-docs-mutation-and-smoke-closure.md`
- `references/frozen-worktree-snapshot-review.md`
- `references/gated-delivery-failclosed-verification-lessons.md`
- `references/guarded-generic-agent-adapter-and-completion-discipline.md`
- `references/hash-bound-snapshot-canonicalization.md`
- `references/hook-preserving-delivery-mutations.md`
- `references/immutable-review-and-runtime-closure-lessons.md`
- `references/immutable-review-finding-followup-lessons.md`
- `references/immutable-review-preflight-and-pycache-hygiene.md`
- `references/immutable-review-repair-and-suite-closure.md`
- `references/immutable-review-runtime-closure-and-provenance.md`
- `references/independent-exact-authority-review-lessons.md`
- `references/independent-pre-freeze-state-integrity-probes.md`
- `references/iteration-continuity-and-delivery-authority-lessons.md`
- `references/iterative-freeze-review-repair-loops.md`
- `references/live-finalization-security-review.md`
- `references/nested-private-runtime-and-doc-closure.md`
- `references/nested-private-runtime-rebundle-and-marker-probes.md`
- `references/opencode-guarded-adapter-litmus-lessons.md`
- `references/opencode-standalone-adapter-proof-audit.md`
- `references/operation-dispatchability-doc-consistency.md`
- `references/policy-convergence-and-exact-delivery-lessons.md`
- `references/polyglot-dispatch-closure-and-exact-provenance.md`
- `references/polyglot-scanner-adversarial-review-and-harness-lessons.md`
- `references/postflight-verifier-integrity-ordering.md`
- `references/postmerge-success-only-live-closeout.md`
- `references/pre-freeze-mode-operation-and-executable-closure-probes.md`
- `references/private-runtime-closure-and-policy-inventory-hardening.md`
- `references/private-runtime-writer-auth-and-positive-controls.md`
- `references/process-scoped-artifact-closing-qa.md`
- `references/production-doc-adr-skill-consistency-audit.md`
- `references/production-fail-closed-seams-and-freeze-provenance.md`
- `references/progressive-bootstrap-and-stacked-pr-grind-lessons.md`
- `references/r90-second-order-helper-and-bounded-ownership-review-lessons.md`
- `references/read-only-exact-merge-executor-review.md`
- `references/real-agent-smoke-diagnostics.md`
- `references/relay-worktree-verification-and-remaining-work-lessons.md`
- `references/restored-session-lineage-and-provider-retry.md`
- `references/review-boundary-and-reviewer-runner-lessons.md`
- `references/review-finding-red-green-toctou-and-quiet-descendants.md`
- `references/reviewer-finding-adjudication-and-process-lifecycle.md`
- `references/reviewer-harness-false-negative-avoidance.md`
- `references/runtime-closure-reseal-and-proof-hygiene.md`
- `references/sealed-transition-slicing-and-tree-verification.md`
- `references/security-egress-finalization-review.md`
- `references/security-finalization-completion-discipline.md`
- `references/shell-dispatch-scanner-composition-and-review-lane-closure.md`
- `references/stacked-ci-and-host-attestation-models.md`
- `references/staged-diff-backstop-audit-lessons.md`
- `references/staged-diff-backstop-drift-and-finalization-test-lessons.md`
- `references/subprocess-write-and-pin-hardening.md`
- `references/successful-child-containment-and-verify-postflight.md`
- `references/trusted-runtime-frozen-review-hardening.md`
- `references/trusted-runtime-private-config-lessons.md`
...
```text
Hermes = recognition, Phase 0 discovery, JIT source reads, read-only status, relay/router/verifier, user interaction, notification, and explicit Delivery Mode operation under gates
Busdriver/Claude Code = sole canonical finalization authority, workflow gates, reviews, MCP/plugin routing, and canonical pipeline semantics
Pi = sole executor route metadata; production dispatch blocked by `agent_containment_and_credential_broker_unavailable`
Codex = fallback coder metadata and PR lead; never raw or production-dispatched for repo-changing work
OpenCode = non-executor historical/comparison evidence only; no current implementation route
Claude Code = authority / backstop path
Grok = Hermes-side relay.review.fast read-only reviewer / PR-comment triage, not Busdriver authority when invoked by Hermes
Gemini = Hermes-side relay.review.long_context read-only architecture/spec reviewer, not Busdriver authority
Cursor = manual IDE sidecar; treat edits as human/manual dirty tree requiring Hermes/Busdriver verification
...
**User expectation for repo-changing work:** all ordinary repo implementation should use this split. Hermes remains the relay/router/verifier and explicit Delivery Mode operator, while Busdriver/Claude remains the sole canonical finalization authority. Pi is the sole executor route metadata, Codex is fallback coder metadata and PR lead, OpenCode is non-executor historical/comparison evidence, and Cursor is the manual IDE sidecar. Route selection does not dispatch work. Direct Hermes edits are a temporary, explicitly named bootstrap/emergency exception only when building or repairing the relay/pipeline itself and the approved draft-worker path is not yet available; do not let that exception become the default workflow.

**Worktree isolation preference for relay implementation:** when `hermes-busdriver-relay` already has WIP or a dirty primary worktree, do not start a new implementation slice in that worktree. Create/select a separate git worktree and branch for the new slice, record the worktree path in run artifacts/briefs, and leave existing relay work untouched unless the user explicitly says to continue in the active worktree.
...
- asks about Pi-only executor routing, Codex fallback/PR-lead metadata, OpenCode non-executor history, the current production agent blocker, or the Claude/Busdriver backstop;
...
**Current User Scope Policy (embedded preference):** Busdriver/Claude is the sole canonical workflow and finalization authority; `hermes-busdriver-relay` is the relay/router/verifier and explicit Delivery Mode operator. Pi is the sole executor route metadata, Codex is fallback coder metadata and PR lead, OpenCode is non-executor historical/comparison evidence, and Cursor is the manual IDE sidecar. All relay roles remain non-dispatchable and unverified because no executable relay-role dispatcher exists. Preserve `avoid_coding_agent_for_review=true`. Do not put relay role config in `~/.claude/busdriver.json`; use relay config (default `~/.hermes/busdriver-relay/config.json`). Do not make Busdriver itself Claude-free unless the user explicitly asks; label relay-side equivalents as metadata, not Busdriver-native Claude runtime. When the user proposes a relay role change or asks why to add/replace an agent, verify live/docs-backed capability first, then answer.
...
- enable any non-Pi mutating executor; OpenCode is explicitly excluded.
...
### Untrusted-repository Git observations

Detailed reusable checklist: `references/git-observation-sandbox-lessons.md`.

Treat every read-only Git probe against an untrusted repository as an executable and network-capable operation, not as a passive file read. Repository config, attributes, submodule config, and promisor/lazy-fetch state can select programs or transports.

For production observation paths:
...
2. Use command-scope inert pins for fixed executable settings (`core.fsmonitor`, hooks, signature programs, diff/textconv, editor/askpass/credentials), disable protocols, and set `GIT_NO_LAZY_FETCH=1` plus an empty `GIT_ALLOW_PROTOCOL`.
3. Pass `--ignore-submodules=none` to status probes so gitlink/submodule drift remains visible, and `--no-ext-diff --no-textconv` to diff probes.
4. In the Pi filesystem broker, audit effective local **and worktree** dynamic `filter.*.{clean,smudge,process}` and `diff.*.{command,textconv}` keys through the descriptor-bound `.git` anchor before each allowed Git verb; keep the sandbox for rename/ABA races.
5. Fail closed if the authenticated sandbox or real Git source is unavailable. Do not fall back to PATH, a user-writable retained copy, or an unsandboxed probe.
...
Keep the distinction explicit: **draft relay surface** remains non-finalizing, but **Hermes Delivery Mode** may have a narrow per-operation mutating executor once the repo implements Busdriver-approved/equivalent evidence checks. Mutating executor runs must acquire a finalization lock, re-check fresh litmus/pre-PR/PR-grind evidence immediately before each side effect, record a redacted `hermes-busdriver-mutating-delivery-run/v0` transcript, and keep persisted/reusable status authority default-deny so old artifacts cannot authorize future mutations. Marker production must use Busdriver-owned trusted writer commands; Hermes must not raw-write `.claude/*` markers. When updating docs/status, remove stale wording that makes read-only status surfaces and operator Delivery Mode sound contradictory. See `references/pr33-completion-status-lessons.md` and `references/gated-finalization-executor-opencode-lessons.md`. When the next dogfood slice is finalization expansion, start with an ADR/integration-contract PR (e.g. repo `ADRs/0005-finalization-authority-integration-contract.md`) that defines authority sources, schema/evidence, marker ownership, fail-closed conditions, and non-goals before any mutating executor code. A safe follow-up is a read-only contract-status/capability-matrix helper that maps ADR unlock criteria to `policy_blocked` remaining-work rows while keeping every authority/capability flag false; see `references/pr35-finalization-contract-status-lessons.md`.

Default relay draft mode still cannot finalize. Production Pi dispatch is `policy_blocked` by `agent_containment_and_credential_broker_unavailable`; OpenCode is parser-rejected and absent from the production manifest. Caller-supplied verifier execution is blocked by `verifier_containment_unavailable`. Push, PR creation, and merge are separately blocked by `atomic_push_base_binding_unavailable`, `atomic_pr_create_binding_unavailable`, and `atomic_merge_base_binding_unavailable`; explicit user intent does not override any blocker. When a relay slice is fully merged/clean and the user says “continue” / “繼續”, choose the next smallest safe read-only/status/integration slice from current repo docs and Phase-0 evidence, then proceed through the same gates; do not pause to ask for a next task unless the next slice has a real product/scope trade-off. When the user explicitly tells Hermes to **complete all work** (commit, PR, merge, or similar delivery language), Hermes must not stop at a dirty tree and must not skip Busdriver's litmus/pre-PR or PR feedback loop. If the user also says to **use subagents** or complains that Hermes is doing the work inline (for example “不是你叫 subagent 做嗎”), stop inline implementation immediately: dispatch the next blocker-fix/implementation slice to subagents and keep main Hermes as operator/verifier/finalizer. Use a split such as reviewer triage + scoped fixer + read-only PR/status checker; subagents must not commit/push/merge. Re-read files after they return and verify with tools. While the active atomic-binding blockers remain, do not push, create a PR, or merge; route those steps to Busdriver/Claude. The following checklist is target-state guidance only after the corresponding production blockers have been removed by a reviewed implementation:
...
This target-state checklist does not grant current commit/PR/merge authority to draft agent launchers or override any active blocker. Under the present policy, Hermes stops before push/PR/merge and routes finalization to Busdriver/Claude; if a future reviewed executor unlocks an operation, every listed check remains mandatory.
...
| Draft agent implementation seam | Agent may modify working tree but cannot finalize | The Pi adapter contract is proven only in a non-installed harness. Production Pi dispatch is blocked by `agent_containment_and_credential_broker_unavailable`; OpenCode has no executor route. |
...
- `scripts/hermes-busdriver-lock`: Token-only release with no force bypass. It atomically retires the canonical lock path to a non-active tombstone, revalidates the moved token/generation, and guarantees no recursive pathname deletion; non-cooperative replacements are restored or preserved.
- `scripts/hermes-busdriver-gate`: Hermes-owned preflight/postflight state checker. Production caller-supplied verifier execution fails closed with `verifier_containment_unavailable`; only the non-installed verifier harness exercises executable-verifier contracts. Commit/push/PR/merge remain false.
- `config/trusted-runtime-manifest.json`: Reviewed trust inventory binding Busdriver commit and package version, external executable/plugin bytes, embedded helper pins, and all side-effect-capable or agent-facing production entrypoints. `production_entrypoints` is an explicit ownership scope, not a claim that listed blocked launchers are dispatchable. Update every consumer constant and digest atomically; the trusted-runtime contract rejects identity or entrypoint drift.
- `scripts/hermes-busdriver-status`: Reports relay-owned routing metadata for the 19-role map. `coding_agent=pi` and the implementation primary/secondary Pi aliases plus Codex fallback metadata are policy-fixed; only non-implementation metadata routes remain configurable. OpenCode is rejected from every current relay route, and Cursor remains the manual IDE. Every role explicitly reports `programmatic_dispatch_allowed=false` and `adapter_verified=false` with a precise blocker because no production relay-role dispatcher exists. Status reports `role_policy=pi_only_executor_codex_fallback_opencode_non_executor`. Relay config shape is root-level `{ "coding_agent": "pi", "avoid_coding_agent_for_review": true, "routes": { "relay.impl.primary": ["pi"], "relay.impl.secondary": ["pi"], "relay.impl.fallback": ["codex"], "relay.pr.lead": ["codex"], "relay.ide.manual": ["cursor"] } }`; invalid/malformed config remains fail-closed.
- `scripts/hermes-busdriver-relay-role`: Read-only resolver for a single relay equivalent role. A resolved role is metadata-only: exit 0 / `ok=true` may confirm a valid selected-agent shape, but root and nested `decision` always keep `dispatch_allowed=false`, `mutation_allowed=false`, `finalization_allowed=false`, and `not_busdriver_native_claude_runtime=true`. Any positive programmatic-dispatch claim fails closed. It must invoke `scripts/hermes-busdriver-status` as a subprocess (not via `importlib`/`SourceFileLoader`) and parse its stdout JSON; unknown/degraded/malformed config, malformed status output, timeouts/failures, and invalid CLI invocations exit nonzero. See `references/relay-role-resolver-lessons.md` and `references/relay-role-status-integration-lessons.md`.
- `scripts/hermes-busdriver-agent-draft`: Production launcher/parser that fails closed immediately after argument parsing, before repository, HOME/state, credential, lock, prompt, gate, run-directory, or worker handling, with `agent_containment_and_credential_broker_unavailable`. Non-installed fixtures retain schema/scope adapter execution tests; they are not production routes. It has no Codex/custom mutation route.
...
- `scripts/hermes-busdriver-agent-smoke`: Parser/authority-negative Pi smoke surface. Production Pi launch is policy-blocked; OpenCode and custom mutation routes are parser-rejected.
...
- `scripts/hermes-busdriver-deliver`: Fail-closed Delivery Mode dispatcher. Plan mode is read-only. Verify-only execution fails with `verifier_containment_unavailable` before delivery-status, PR-grind, GitHub auth, repo helper, or caller-command paths; `execute --operation pr-grind` may wrap the read-only bounded PR-grind loop and write a Hermes-owned run artifact while keeping top-level reusable authority false. Parser exposure is not dispatchability: `pre-pr-review` returns `isolated_review_runtime_unavailable` before delivery-status, repository/state/lock, artifact, credential, or trusted-writer handling; `push` returns `atomic_push_base_binding_unavailable`; `pr-create` returns `atomic_pr_create_binding_unavailable`; and `merge` returns `atomic_merge_base_binding_unavailable`. These early blockers must report the finalization lock as skipped, must not synthesize run ID/timestamp state or persist artifacts, and must never be bypassed with direct Git/GitHub commands. Only an operation without an active fixed blocker may acquire the Hermes finalization lock, re-check fresh evidence, or write a redacted mutating transcript. The dormant Busdriver-writer adapter is not invoked by production `pre-pr-review`, and Hermes must not raw-write `.claude/*` markers. Do not expose no-op mutating operations as executable CLI choices: autonomous PR-grind fix loops must remain out of the mutating `deliver` surface (or explicitly read-only/status-only) until real gated draft-adapter fix semantics, fresh review evidence, and commit/push/re-poll tests exist. Every result carries a durable `hermes-busdriver-delivery-run/v0` envelope (`run_id`, phase, status, reason, repo/PR identity, authority flags, and artifact references); `--run-id` provides a stable run identity for operator/subagent/cron handoff while artifact filenames still add timestamp/PID uniqueness. `--mode status --run-id <id>` is a read-only lookup of the latest valid matching Hermes-owned run artifact; it validates versioned deliver/run envelopes plus fail-closed decision/authority metadata, preserves artifact repo/PR identity, returns artifact path plus sanitized metadata instead of verifier output tails, and does not run delivery-status, verifiers, PR-grind, or repo mutations. When deliver wraps delivery-status and forwards nested PR-grind/litmus/relay-role/Phase-0 drift-status timeouts, its own default/effective timeout must cover those nested budgets plus margin so delivery-status can emit structured fail-closed output rather than being killed by the wrapper; forward `--relay-role`/`--relay-config` with `--relay-role-timeout` only when relay-role evidence is requested, and `--drift-baseline` with `--phase0-status-timeout` only when drift evidence is requested. See `references/deliver-run-envelope-lessons.md`, `references/delivery-litmus-pr-mode-lessons.md`, `references/gated-finalization-executor-opencode-lessons.md`, and `references/delivery-mode-dogfood-litmus-lessons.md`.
...
- `scripts/hermes-busdriver-finalization-contract-status`: Read-only ADR 0005/0008 contract/capability matrix helper. It maps finalization surfaces to lifecycle rows: gated executor/envelope rows may be `implemented_gated`, while programmatic dual-review, autonomous PR-grind fix/push/re-poll, and marker interop/write rows remain `policy_blocked`. It must keep every reusable authority/capability flag false (`capability_allowed=false`, `safe_to_execute_by_this_helper=false`, all commit/push/PR/merge/marker flags false), and it must not run dispatchers, inspect/mutate target repos, write markers, or make old artifacts reusable authority. Lifecycle booleans such as `implemented=true` are not permission flags; recursive false-authority tests should not reject them as authority-positive. When adding top-level status row lists such as `authority_sources`, preserve compatibility fields like `required_authority_sources`, keep `current_evidence` empty unless a later approved helper actually verifies it, and extend recursive false-authority tests to cover permission fields; see `references/adr0005-authority-source-status-lessons.md` and `references/finalization-executor-status-sync-lessons.md`.
- `scripts/hermes-busdriver-relay-brief`: Read-only compact status/roadmap helper for Telegram-style summaries. Its catalog contains four active tasks plus the Pi executor-route blocker row, and a fail-closed `next_safe_slice`; all commit/push/PR/merge/finalization/marker-write/programmatic-execution/non-Pi-executor authority flags remain false.
...
Pi is the sole executor route metadata, not an enabled production launcher. Codex remains fallback coder/PR-lead metadata and OpenCode remains non-executor historical/comparison evidence. The non-installed Pi/OpenCode harnesses prove adapter schemas, scope enforcement, reconciliation, and authority-negative results only. Production `hermes-busdriver-gate` and `hermes-busdriver-agent-draft` stop before worker launch or credential copying with `agent_containment_and_credential_broker_unavailable`.

Production caller-supplied verifier execution is independently blocked by `verifier_containment_unavailable`. A passing set of state checks does not override either policy blocker, and parser/envelope implementation does not constitute dispatch authority.

Use production agent commands only as expected-blocked capability probes. For actual implementation while the blocker is active, route to Busdriver/Claude or use an explicitly approved bootstrap/emergency path; do not claim Pi/OpenCode production dispatch, real-agent smoke proof, or a resulting `needs_busdriver_review` draft.

Any future production adapter promotion requires reviewed containment and credential brokering plus a fresh policy decision. A successful draft must still end at `needs_busdriver_review` with commit/push/PR/merge/marker/deploy/release/publish/finalization authority false.
...
The non-installed adapter harness uses PATH guards to shadow finalization commands (`git commit`, `git push`, `gh pr create`, `gh pr merge`). This is a harness-contract lesson, not an enabled production draft run. Run full contract suites from Hermes/operator verification; do not describe harness verifier behavior as production caller-verifier capability.
...
4. production `hermes-busdriver-gate` state/policy probes, expected to block agent and caller-verifier dispatch;
5. `hermes-busdriver-agent-draft` parser/fail-closed Pi production responses plus non-installed Pi contract and historical OpenCode fixture tests;
...
7. read-only `hermes-busdriver-relay-brief` for compact idle/blocked/next-safe-slice summaries without finalization authority;
8. `hermes-busdriver-agent-smoke` parser/authority-negative checks; historical real-agent smoke is provenance only, not production containment proof;
9. explicit `hermes-busdriver-deliver` parser/result-envelope surfaces, with verifier execution, push, PR-create, and merge non-dispatchable under their current policy blockers;
10. advisory/user-facing routing that tells the user when finalization still needs Claude Code / Busdriver or stronger equivalent gates.

Not allowed yet:
- production Pi dispatch or credential copying while `agent_containment_and_credential_broker_unavailable` is active; OpenCode has no production executor surface;
- caller-supplied verifier execution while `verifier_containment_unavailable` is active;
- direct Git push or any bypass of the Delivery Mode `atomic_push_base_binding_unavailable` fail-closed guard;
- direct PR creation or any bypass of `atomic_pr_create_binding_unavailable`;
- direct merge or any bypass of `atomic_merge_base_binding_unavailable`;
...
- raw `.claude/*` marker writes, marker forging/deletion/consumption, or treating marker filenames as authority;
- autonomous PR-grind fix/push/re-poll loops that bypass gated draft adapters and fresh review evidence;
- deploy/release/publish automation;
- direct MCP/plugin routing;
- claim that Hermes-launched work is gate-safe without hook-runtime equivalence or the gated Delivery Mode evidence checks;
- activating any non-Pi executor route; OpenCode is explicitly non-executor.
...
When maintaining compact relay status helpers or cleanup support, consult `references/pr98-roadmap-brief-cleanup-lessons.md` and `references/pr99-positive-helper-followup-lessons.md`. Key rules: fail closed when repo Git state, repo skill source, installed skill path, or contract-status evidence is unverified; run contract-status evidence from the requested repo root; validate nested helper schema/read_only before treating it as trusted evidence; strip inherited `GIT_*` variables and use `git --no-optional-locks` plus config-stable, colorless `--porcelain=v1` untracked-file status probes for read-only dirty checks; preserve the two-column porcelain leading columns; surface missing/extra/diff skill drift separately with the correct reconciliation direction; derive brief roadmap text from the canonical task list; use phase-appropriate verification when the repo is intentionally dirty; use relay-owned bounded PR-grind polling instead of ad hoc foreground sleep loops; and restart latest-head PR-grind after every reviewer-fix push before any merge, resolving addressed review threads only after code/test evidence inside explicit PR-grind finalization.
...
- [ ] Keep Pi as the sole executor route metadata, Codex as fallback coder/PR-lead metadata, OpenCode as non-executor historical/comparison evidence, Cursor manual, and every role non-dispatchable.
