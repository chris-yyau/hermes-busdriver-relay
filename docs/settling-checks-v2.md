...
- the Pi adapter contract plus historical OpenCode parity evidence exercised through non-installed test harnesses; production accepts only Pi and keeps its dispatch `policy_blocked` by `agent_containment_and_credential_broker_unavailable`, while production parsers reject OpenCode;
...
The read-only/status relay surface remains complete. Parser/result-envelope surfaces are not dispatchability: production Pi dispatch, verifier commands, pre-PR review, push, PR creation, and merge remain blocked by `agent_containment_and_credential_broker_unavailable`, `verifier_containment_unavailable`, `isolated_review_runtime_unavailable`, `atomic_push_base_binding_unavailable`, `atomic_pr_create_binding_unavailable`, and `atomic_merge_base_binding_unavailable`; production parsers reject OpenCode rather than exposing a blocked route. Raw marker writes, deploy/release/publish, and autonomous PR-grind fix loops remain intentionally blocked.
